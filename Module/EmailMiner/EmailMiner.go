...
import (
	"GoogleBingExtractor/ConfigManager"
	"GoogleBingExtractor/LogManager"
	"GoogleBingExtractor/Module/EmailChecker"
	"context"
	"fmt"
	"io/ioutil"
	"log"
...
func getHtmlContentWithCtx(ctx context.Context,pageUrl string)string  {
	...
	hReq,err := http.NewRequestWithContext(ctx,"GET",pageUrl,nil)
	if err != nil{
		return ""
	}
...
func (this *EmailMiner)exoploreUrl(ctx context.Context,expUrl string)  {
	defer this.wg.Done()
	html := getHtmlContentWithCtx(ctx,expUrl)
...
func GetEmail(ctx context.Context,website string)[]string  {
	var emailMiner EmailMiner
	return emailMiner.DetectEmail(ctx,website)
}

//采集Email
func (this *EmailMiner)DetectEmail(ctx context.Context,website string)(retList []string)  {
	eUrl,err := url.Parse(website)
	if err != nil {
		LogManager.ErrorLogger.Error("[GetEmail]:", website)
		return retList
	}
	//host := eUrl.Host
